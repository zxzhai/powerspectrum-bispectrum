...
#for i in range(1):
    z = z_c[i]
    Vol = V[i]
    deltak = 2.0*np.pi/Vol**(1.0/3.0) * 2
    kf = deltak
    ...
    def CBB(k1, mu1, k2, mu2, k3, mu3):
        ...
        NB = Vol**2/(8.0*np.pi**4)*k1*k2*k3*(deltak)**3 * 1.0 /(Np*Na)# * (np.pi*4)**(-1.0)*Sintheta   #  their formula should be wrong.
        return sB*Vol*BPmodel.Ptilt(k1, mu1, sigmap, z, b1, f, ng)*BPmodel.Ptilt(k2, mu2, sigmap, z, b1, f, ng)*BPmodel.Ptilt(k3, mu3, sigmap, z, b1, f, ng)/ NB
